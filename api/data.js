...
//   GET  /api/data  -> retorna { funcionarios, meses }
//   POST /api/data  -> salva   { funcionarios, meses }
// Segurança: se ACCESS_CODE estiver definido, exige o header x-access-code.
...
module.exports = async (req, res) => {
  ...
  if (!KV_URL || !KV_TOKEN) { res.status(500).json({ error: 'banco_nao_configurado' }); return; }
  try {
    if (req.method === 'GET') {
      const out = await kv(['GET', DATA_KEY]);
      const data = out && out.result ? JSON.parse(out.result) : { funcionarios: [], meses: [] };
